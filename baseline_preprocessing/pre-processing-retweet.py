...
df = pd.read_csv('data/origin_data/retweet_list.csv',sep="\t")
...
print(len(g.nodes))
print(len(g.edges))
df_label.to_csv("data/retweet_labels_baseline.txt", header = False, columns = ['twitter_id','party'], sep=' ', index = False)
df.to_csv("data/retweet_list_baseline.csv", header = False, columns = ['user_id','retweeted'], sep=' ', index = False)