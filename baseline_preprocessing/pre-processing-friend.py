...
df = pd.read_csv('data/origin_data/friend_list.csv',sep="\t")
g = nx.from_pandas_edgelist(df, "follower", "followee", edge_attr=True)
df_label = df_label[
...
print(len(g.nodes))
print(len(g.edges))
df_label.to_csv("data/friend_labels_baseline.txt", header = False, columns = ['twitter_id','party'], sep=' ', index = False)
df.to_csv("data/friend_list_baseline.csv", header = False, columns = ['follower','followee'], sep=' ', index = False)