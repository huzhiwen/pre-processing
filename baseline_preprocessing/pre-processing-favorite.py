...
df = pd.read_csv('data/origin_data/favorite_list.csv',sep="\t")
g = nx.from_pandas_edgelist(df, "user_id", "liked", edge_attr=True)
df_label = df_label[
...
print(len(g.nodes))
print(len(g.edges))
df_label.to_csv("data/favorite_labels_baseline.txt", header = False, columns = ['twitter_id','party'], sep=' ', index = False)
df.to_csv("data/favorite_list_baseline.csv", header = False, columns = ['user_id','liked'], sep=' ', index = False)